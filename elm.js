...
Elm.EntityRow = Elm.EntityRow || {};
Elm.EntityRow.make = function (_elm) {
   "use strict";
   _elm.EntityRow = _elm.EntityRow || {};
   if (_elm.EntityRow.values) return _elm.EntityRow.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   ...
   $String = Elm.String.make(_elm);
   var _op = {};
   var maybeCell = F2(function (maybe,renderer) {
      var _p0 = maybe;
      if (_p0.ctor === "Just") {
            return renderer(_p0._0);
         } else {
   ...
   var view = F2(function (address,model) {
      return A2($Html.tr,
      _U.list([]),
      _U.list([A2($Html.td,_U.list([]),_U.list([$Html.text(model.name)]))
              ,A2($Html.td,_U.list([]),_U.list([$Html.text(A2($String.join,", ",model.types))]))
              ,A2($Html.td,_U.list([]),_U.list([A2(maybeCell,model.description,function (desc) {    return $Html.text(desc);})]))
              ,A2($Html.td,
              _U.list([]),
              _U.list([A2(maybeCell,model.imageContentUrl,function (url) {    return A2($Html.img,_U.list([$Html$Attributes.src(url)]),_U.list([]));})]))
              ,A2($Html.td,_U.list([]),_U.list([A2(maybeCell,model.detailedDescription,function (detail) {    return $Html.text(detail.articleBody);})]))
              ,A2($Html.td,
              _U.list([]),
              _U.list([A2(maybeCell,
              model.detailedDescription,
              function (detail) {
                 return A2($Html.a,_U.list([$Html$Attributes.href(detail.url),$Html$Attributes.target("_blank")]),_U.list([$Html.text("Wikipedia Article")]));
              })]))
              ,A2($Html.td,_U.list([]),_U.list([$Html.text($Basics.toString(model.resultScore))]))]));
   });
   var update = F2(function (action,model) {    return {ctor: "_Tuple2",_0: model,_1: $Effects.none};});
   var NoOp = {ctor: "NoOp"};
   var Model = F6(function (a,b,c,d,e,f) {    return {name: a,types: b,description: c,imageContentUrl: d,detailedDescription: e,resultScore: f};});
   var DetailedDescription = F2(function (a,b) {    return {articleBody: a,url: b};});
   return _elm.EntityRow.values = {_op: _op,DetailedDescription: DetailedDescription,Model: Model,NoOp: NoOp,update: update,view: view,maybeCell: maybeCell};
};
Elm.EntityTable = Elm.EntityTable || {};
Elm.EntityTable.make = function (_elm) {
   "use strict";
   _elm.EntityTable = _elm.EntityTable || {};
   if (_elm.EntityTable.values) return _elm.EntityTable.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   $EntityRow = Elm.EntityRow.make(_elm),
   $Html = Elm.Html.make(_elm),
   $Html$Attributes = Elm.Html.Attributes.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   ...
   var NoOp = {ctor: "NoOp"};
   var viewRow = F2(function (address,entityModel) {    return A2($EntityRow.view,A2($Signal.forwardTo,address,$Basics.always(NoOp)),entityModel);});
   var view = F2(function (address,model) {
      var rows = A2($List.map,viewRow(address),model);
      return A2($Html.table,
      _U.list([$Html$Attributes.$class("mui-table")]),
      _U.list([A2($Html.thead,
              _U.list([]),
              _U.list([A2($Html.tr,
              _U.list([]),
              _U.list([A2($Html.th,_U.list([]),_U.list([$Html.text("Name")]))
                      ,A2($Html.th,_U.list([]),_U.list([$Html.text("Types")]))
                      ,A2($Html.th,_U.list([]),_U.list([$Html.text("Description")]))
                      ,A2($Html.th,_U.list([]),_U.list([$Html.text("Image")]))
                      ,A2($Html.th,_U.list([]),_U.list([$Html.text("Detailed Description")]))
                      ,A2($Html.th,_U.list([]),_U.list([$Html.text("URL")]))
                      ,A2($Html.th,_U.list([]),_U.list([$Html.text("Result Score")]))]))]))
              ,A2($Html.tbody,_U.list([]),rows)]));
   });
   var init = function (model) {    return {ctor: "_Tuple2",_0: model,_1: $Effects.none};};
   return _elm.EntityTable.values = {_op: _op,init: init,NoOp: NoOp,update: update,view: view,viewRow: viewRow};
};
Elm.SearchBox = Elm.SearchBox || {};
Elm.SearchBox.make = function (_elm) {
   "use strict";
   _elm.SearchBox = _elm.SearchBox || {};
   if (_elm.SearchBox.values) return _elm.SearchBox.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   ...
   var update = F2(function (action,model) {
      var _p1 = action;
      if (_p1.ctor === "Query") {
            return {ctor: "_Tuple2",_0: _U.update(model,{query: _p1._0}),_1: $Effects.none};
         } else {
...
Elm.EntityTableSearchContainer.make = function (_elm) {
   "use strict";
   _elm.EntityTableSearchContainer = _elm.EntityTableSearchContainer || {};
   if (_elm.EntityTableSearchContainer.values) return _elm.EntityTableSearchContainer.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   $EntityRow = Elm.EntityRow.make(_elm),
   $EntityTable = Elm.EntityTable.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   $SearchBox = Elm.SearchBox.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var decoder = A7($Json$Decode.object6,
   $EntityRow.Model,
   A2($Json$Decode.at,_U.list(["result","name"]),$Json$Decode.string),
   A2($Json$Decode.at,_U.list(["result","@type"]),$Json$Decode.list($Json$Decode.string)),
   $Json$Decode.maybe(A2($Json$Decode.at,_U.list(["result","description"]),$Json$Decode.string)),
   $Json$Decode.maybe(A2($Json$Decode.at,_U.list(["result","image","contentUrl"]),$Json$Decode.string)),
   $Json$Decode.maybe(A2($Json$Decode.at,
   _U.list(["result","detailedDescription"]),
   A3($Json$Decode.object2,
   $EntityRow.DetailedDescription,
   A2($Json$Decode._op[":="],"articleBody",$Json$Decode.string),
   A2($Json$Decode._op[":="],"url",$Json$Decode.string)))),
   A2($Json$Decode.at,_U.list(["resultScore"]),$Json$Decode.$float));
   var decodeData = A2($Json$Decode.at,_U.list(["itemListElement"]),$Json$Decode.list(decoder));
   _op["=>"] = F2(function (v0,v1) {    return {ctor: "_Tuple2",_0: v0,_1: v1};});
   var queryUrl = function (query) {
      return A2($Http.url,
      "https://kgsearch.googleapis.com/v1/entities:search",
      _U.list([A2(_op["=>"],"query",query)
              ,A2(_op["=>"],"key","AIzaSyC-CzQ4NzurDdc9d7ItDLlUi--K0KDC9ZI")
              ,A2(_op["=>"],"limit","10")
              ,A2(_op["=>"],"indent","false")]));
   };
...
Elm.KnowledgeGraph = Elm.KnowledgeGraph || {};
Elm.KnowledgeGraph.make = function (_elm) {
   "use strict";
   _elm.KnowledgeGraph = _elm.KnowledgeGraph || {};
   if (_elm.KnowledgeGraph.values) return _elm.KnowledgeGraph.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   $EntityTableSearchContainer = Elm.EntityTableSearchContainer.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var app = $StartApp.start({init: $EntityTableSearchContainer.init
                             ,view: $EntityTableSearchContainer.view
                             ,update: $EntityTableSearchContainer.update
                             ,inputs: _U.list([])});
   var main = app.html;
   var tasks = Elm.Native.Task.make(_elm).performSignal("tasks",app.tasks);
   return _elm.KnowledgeGraph.values = {_op: _op,main: main};
};
